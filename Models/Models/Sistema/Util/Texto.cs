...
namespace Util
{
    public static class Texto
    {
        ...
        public static string GetConteudoEntre(string texto, string str1, string str2)
        {
            return GetConteudoEntre(texto, str1, str2, str1, 0);
        }

        public static string GetConteudoEntre(string texto, string str1, string str2, int aux)
        {
            return GetConteudoEntre(texto, str1, str2, str1, aux);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetConteudoEntre(string texto, string str1, string str2, string str1Comparar, int aux)
        {
            if (texto.IndexOf(str1) == -1)
                return null;

            var inicio = texto.IndexOf(str1) + str1.Length;
            var fim = texto.IndexOf(str2, aux);
            var tamanho = texto.Substring(inicio).IndexOf(str2, (aux != 0)?aux - inicio: 0);

            var result = string.Empty;

            if (tamanho != -1) {
                result = texto.Substring(inicio, tamanho);
            }

            if (result.IndexOf(str1Comparar) != -1 && aux != 0)
            {
                return GetConteudoEntre(texto, str1, str2, fim + 1);
            }

            if (aux != 0)
                result = texto.Substring(inicio, aux - inicio - 1);
            else
                result = texto.Substring(inicio, tamanho);
        ...
        public static string ReplaceChavesGetConteudoEntre(string texto, string str1, string str2){

            return GetConteudoEntre(texto, str1, str2, "{", 0);
        ...
        public static string ReplaceChaves(string texto, object chavesValor)
        {
            ...
            foreach (PropertyInfo p in sourceProperties)
            {
                ...
                var chaveCondicional = "[" + nome + "{";

                if (texto.IndexOf(chaveCondicional) != -1 && (valor == null || valor.ToString() == string.Empty))
                {
                    texto = texto.Replace(chaveCondicional + ReplaceChavesGetConteudoEntre(texto, chaveCondicional, "}]") + "}]", string.Empty);
                }

                texto = texto.Replace(chaveCondicional + ReplaceChavesGetConteudoEntre(texto, chaveCondicional, "}]") + "}]", ReplaceChavesGetConteudoEntre(texto, chaveCondicional, "}]"));

                //texto = texto.Replace(chaveCondicional, string.Empty);
